// Sean Allen
// CSCI 221 HW 12: TSP Part 4

// The goal was to parallelize ga_search(). I did that below,
//   but it doesn't work and I have absolutely no idea what's wrong.
...
void helper ( Cities cities, Cities::permutation_t best_ordering, Cities::permutation_t ordering, 
        double best_dist, int i, int pop_size, std::mutex mutex)
{
...
Cities::permutation_t(cities.size())
          ...
          unsigned nthread = 1)
{
  ...
  std::vector<TournamentDeme*> all_demes;

  const unsigned           deme_size = pop_size / nthread
            ,     smallest_deme_size = pop_size % nthread
            ;

  while (pop_size > smallest_deme_size) {
      for (int i=0; i<deme_size; ++i) {
          all_demes.push_back(new TournamentDeme (&cities, deme_size, mutation_rate));
      }
    pop_size -= deme_size;
  }

  for (int i=0; i<smallest_deme_size; ++i)
      all_demes.push_back(new TournamentDeme (&cities, smallest_deme_size, mutation_rate));
    

  // Evolve the population to make it fitter and keep track of
  // the shortest distance generated.

  for (auto deme : all_demes) {
    for (long i = 1; i <= iters/deme_size; ++i) {
